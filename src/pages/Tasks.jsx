...
const Tasks = () => {
  ...
  const pendingTask =data?.data?.filter(task=>task.status==='pending')
  const runningTask = data?.data?.filter(task=>task.status==='running')
  const doneTask = data?.data?.filter(task=>task.status==='done')

  

  console.log(data?.data);
  // const archiveTask = tasks.filter(task=>task.status==='archive')
      ...
      <div className="col-span-9 px-10 pt-10">
        ...
        <div className="grid grid-cols-3 gap-5 mt-10">
          <div className="relative h-[800px] overflow-auto">
            ...
            { 
           pendingTask?.map(task =>  < TaskCard key={task.id} task={task} />)
